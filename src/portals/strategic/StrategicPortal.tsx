...
// FIX: Removed unused 'useCallback' and 'MciState' imports.
...
const StrategicPortal = ({ activePortal, setActivePortal, onGoToIntro }: { activePortal: Portal, setActivePortal: (p: Portal) => void, onGoToIntro: () => void }) => {
    // FIX: Only importing used context variables.
    ...
    // FIX: Removed unused state variables (mciRegion, mciConfirmText, showToast definition)
    const [showCriticalModal, setShowCriticalModal] = useState(false);
    const [lastUpdated, setLastUpdated] = useState(new Date().toLocaleTimeString());
    const [toast, setToast] = useState<{message: string | null, type: string | null}>({ message: null, type: null });
    const modalRef = useRef(null);

    // FIX: Removed unused local showToast function (TS6133)
